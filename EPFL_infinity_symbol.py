...
# ================================================================
# Data: Infinity Symbol
# ================================================================
def generate_infinity_symbol_data(n_samples=1000, noise_level=0.1, grid_size=63):
    """Generate infinity symbol (lemniscate of Bernoulli) with noisy and clean coordinates"""
    t = np.linspace(0, 2 * np.pi, grid_size ** 2)
    ...
    X_clean = torch.tensor(X_clean, dtype=torch.float32).reshape(grid_size, grid_size, 2)
    X_noisy = torch.tensor(X_noisy, dtype=torch.float32).reshape(grid_size, grid_size, 2)
...
def train_distance_net(net, Xg, d_true, epochs=500, lr=1e-3, wd=1e-6, batch_size=512):
...
def train_model_wcrr(model, dataloader, epochs=1000, lr=1e-3, save_path="trained_models/wcrr_infinity.pth"):
...
def visualize_all(*, X_noisy, X_clean, dd_true, dd_nn, dd_icnn, dd_wcrr, xx, yy, save_path="EPFL_infinity_compare.png"):
...
def plot_all_losses(losses_dict, save_path="EPFL_infinity_all_losses.png"):
    plt.figure(figsize=(7, 5))
    for name, losses in losses_dict.items():
        plt.plot(losses, label=name)
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Loss curves (EPFL, Infinity Symbol)")
    plt.ylim(0, 5)  # 限制 y 轴范围
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    print(f"[Viz] Saved combined loss curves to {save_path}")
...
if __name__ == "__main__":
    ...
    # ----- data -----
    X_noisy, X_clean = generate_infinity_symbol_data(grid_size=63)

    # tensors for WCRR training
    X_noisy_img = X_noisy.permute(2, 0, 1).unsqueeze(0)
    X_clean_img = X_clean.permute(2, 0, 1).unsqueeze(0)
    dataset = TensorDataset(X_noisy_img, X_clean_img)
    dataloader = DataLoader(dataset, batch_size=1)

    # ----- WCRR -----
    param_multi_conv = {"size_kernels": [5, 5, 5], "num_channels": [2, 4, 8, 60]}
    param_spline_activation = {"num_activations": 60, "num_knots": 100,
                               "x_min": -2.0, "x_max": 2.0, "init": "leaky_relu",
                               "slope_min": 0.01, "slope_max": None,
                               "antisymmetric": False, "clamp": True}
    param_spline_scaling = {"num_activations": 60, "num_knots": 200,
                            "x_min": -2.0, "x_max": 2.0, "init": "identity",
                            "slope_min": 0.01, "slope_max": None,
                            "antisymmetric": False, "clamp": True}
    ...
    model, wcrr_loss = train_model_wcrr(model, dataloader, epochs=1000, lr=1e-3,
                                        save_path="trained_models/wcrr_infinity.pth")
    ...
    nn_reg = MLP(in_dim=2)
    nn_reg, nn_loss = train_distance_net(nn_reg, Xg, d_true, epochs=1000)
    dd_nn = eval_to_contour(nn_reg, xx, yy)

    # ----- ICNN -----
    icnn = ICNN(in_dim=2)
    icnn, icnn_loss = train_distance_net(icnn, Xg, d_true, epochs=1000, lr=1e-2)
    dd_icnn = eval_to_contour(icnn, xx, yy)

    # ----- Panel -----
    visualize_all(X_noisy=X_noisy, X_clean=X_clean,
                  dd_true=dd_true, dd_nn=dd_nn, dd_icnn=dd_icnn, dd_wcrr=dd_wcrr,
                  xx=xx, yy=yy, save_path="EPFL_infinity_compare.png")

    # ----- Unified Loss Curve -----
    all_losses = {
        "NN": nn_loss,
        "ICNN": icnn_loss,
        "IWCNN/WCRR": wcrr_loss
    }
    plot_all_losses(all_losses, save_path="EPFL_infinity_all_losses.png")
